...
const filters = {
 price: [
  ...
  category: [
    { value: 'hair-growth-treatments', label: 'Hair Growth Treatments', checked: false },
    ...
    { value: 'shampoos-and-cleansers', label: 'Shampoos & Cleansers', checked: false },
  ],
}
const sortOptions = [
  ...
  setSelectedFilters((prev) => {
    ...
    if(filterType === 'price'){
      if(checked){
        ...
        updatedValues = currentValues.filter((v) => v.min !== value.min || v.max !== value.max )
      }
    }
  ...
  return (
    <div className="bg-white">
      <div className="px-4 py-16 text-center sm:px-6 lg:px-8">
        <h1 className="text-4xl font-bold tracking-tight text-gray-900">Hair Regrowth Treatments</h1>
        <p className="mx-auto mt-4 max-w-xl text-base text-gray-500">
          Restore your confidence with clinically proven solutions formulated to stimulate hair follicles and 
        promote visible growth. Explore our range of Minoxidil-based treatments trusted
        by men and women across Kenya.
        </p>
      </div>
                      ...
                      <div className="flex h-5 shrink-0 items-center">
                        <div className="group grid size-4 grid-cols-1">
                          <input
                            ...
                            name="price[]"
                            type="checkbox"
                            checked={selectedFilters.price.some((p) => p.min === option.min && p.max === option.max)}
            ...
            <Menu as="div" className="relative inline-block">
              <div className="flex">
                <MenuButton className="group inline-flex justify-center text-sm font-medium text-gray-700 hover:text-gray-900">
                  ...
                  <span>Sort</span>
                  <span className="ml-1 text-indigo-500">{sortOptions.find(opt => opt.value === sortBy)?.name}</span>
                  <ChevronDownIcon
                    aria-hidden="true"
