...
export default function CartContents() {
  const {cart,setCart,Total,OrderTotal,setShippingCost,selectedCity,setSelectedCity} = useProducts()
  ...
  useEffect(() => {
    ...
    const citydata = allCities.find((cityitem) => cityitem.city === selectedCity)
    if(citydata){
        ...
        <form className="mt-12">
          <div>
            ...
            <ul role="list" className="divide-y divide-gray-200 border-t border-b border-gray-200">
              {cart.length > 0 ? cart.map((product,productIdx) => (
                      ...
                      <div className="mt-4 flex items-center sm:absolute sm:top-0 sm:left-1/2 sm:mt-0 sm:block">
                        <div className="inline-grid w-full max-w-16 grid-cols-1">
                          ...
                          onChange={(e) => {
                            ...
                            setCart((prevcart) => prevcart.map((item) => item.id === product.id ? {...item,quantity : newQty} : item )
              )}}
            ...
            <option key={city} value={city}>
              {city} (Ksh {rate})
                  ...
                  <div className="flex items-center justify-between py-4">
                    ...
                    <dd className="text-base font-medium text-gray-900">Ksh {OrderTotal}</dd>
                  </div>
                </dl>
              </div>
            </div>
