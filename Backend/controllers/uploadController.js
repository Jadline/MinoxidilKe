...
const PACKAGE_UPLOAD_DIR = path.join(__dirname, '..', 'public', 'uploads', 'packages');
...
function publicPathFromFile(file, folder) {
  ...
  return (
    '/' +
    (uploadsIndex >= 0 ? pathSegments.slice(uploadsIndex) : ['uploads', folder, file.filename])
      .join('/')
      .replace(/\\/g, '/')
  );
}
 ...
 * Expects multipart field "image". Returns path like /uploads/products/xxx.jpg
 */
