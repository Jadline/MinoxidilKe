...
async function seedPackages() {
  ...
  if (fs.existsSync(UPLOADS_DIR)) {
    filenames = fs.readdirSync(UPLOADS_DIR).filter((f) => {
      ...
      return ['.jpg', '.jpeg', '.png', '.webp', '.gif'].includes(ext) && f !== '.gitkeep';
    });
  }

  if (filenames.length === 0) {
    console.log('No images found in public/uploads/packages. Add images there and run again.');
    process.exit(0);
  }

  const nextId = await Package.aggregate([{ $group: { _id: null, maxId: { $max: '$id' } } }]).then(
    (r) => (r[0]?.maxId ?? 0) + 1
  );
  ...
  try {
    ...
    if (err.code === 11000) {
      console.log('Some packages already exist (duplicate id). Delete existing packages or use different ids.');
    }
